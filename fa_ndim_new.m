...
function nbest=fa_ndim_new(A,pr,c,w,K,Lb,Ub,x1)
...
for i=1:n,
   ...
   Lightn(i)=cost(ns(i,:),A,pr,c,w,K,x1);               % Evaluate objectives
end
...
for i=1:n,
    for j=1:n,
      ...
      Lightn(i)=cost(ns(i,:),A,pr,c,w,K,x1);           % Call the objective
      % Update moves
...
function z=cost(u,A,pr,b,w,K,x1)
...
z = u'*w*u+sum((u>x1).*(0.02+0.06*(u-x1).*pr)+(u<x1).*(0.02+0.04*(x1-u).*pr))+f;
% -----------------------------------------------------------------------%